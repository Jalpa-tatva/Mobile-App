...
export const useScrollHandlerY = (name: TabName,setShowDefaultHeader:any) => {
  const {
    accDiffClamp,
    focusedTab,
    snapThreshold,
  ...
  const scrollHandler = useAnimatedScrollHandler(
    {
      onScroll: event => {
        if (!enabled.value) return;
        // scrollYCurrent.value = event.contentOffset.y;
        // // Show default header when scrolled past HEADER_HEIGHT
        // if (event.contentOffset.y > 504) {
        //   runOnJS(setShowDefaultHeader)(true);
        // } else {
        //   runOnJS(setShowDefaultHeader)(false);
        // }
...
>(
  (
    ...
    const {scrollHandler, enable} = useScrollHandlerY(name,setShowDefaultHeader);
